...
interface BlackBoxPageProps { // Renamed from InformationAccessPageProps
  onBackToLanding: () => void;
  isLoggedIn: boolean;
  onLogin: () => void;
}

export const BlackBoxPage: React.FC<BlackBoxPageProps> = ({ onBackToLanding, isLoggedIn, onLogin }) => { // Renamed from InformationAccessPage
  const { isDarkMode } = useTheme();
  const [hasAccessedInfo, setHasAccessedInfo] = useState(isLoggedIn); // State to control access
  ...
  // const accentBorder = isDarkMode ? 'border-red-700' : 'border-red-600'; // Not used in this version
  ...
  const renderIntroGate = () => (
    <div className={`relative min-h-screen flex items-center justify-center animate-in fade-in duration-700 p-6 ${pageBg}`}>
        <div className="absolute inset-0 z-0 overflow-hidden">
            <img 
              src={IMAGES.BLACK_BOX_BG} // Updated from INFORMATION_BG
        ...
        <div className={`relative z-20 max-w-3xl w-full border-2 p-8 md:p-12 rounded-lg shadow-2xl flex flex-col
             ...
             {/* Main Message - The Black Box Description */}
             ...
             <div className={`pt-8 border-t ${isDarkMode ? 'border-zinc-800' : 'border-zinc-200'}`}>
                 ...
                 {isLoggedIn ? (
                     ...
                     <div className="text-center">
                         ...
                         <button 
                            onClick={onLogin} // Trigger the login handler
                            className={`w-full py-4 font-bold uppercase tracking-widest text-sm transition-all duration-300 rounded shadow-lg mt-2
                                        ${accentButtonBg} ${accentButtonText}`}
                         >
  ...
  // --- Main Information Content (Only rendered if hasAccessedInfo is true) ---
  return (
    <div className={`relative min-h-screen animate-in fade-in duration-700 transition-colors
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <img 
          ...
          alt="Black Box Background" 
          className="w-full h-full object-cover object-center" 
          style={{ filter: 'brightness(60%) blur(2px)', pointerEvents: 'none' }} 
        />
        <div className={`absolute inset-0 z-10 transition-colors duration-500
                         ${isDarkMode ? 'bg-black/60' : 'bg-white/80'}`}></div>
      </div>
      ...
      <div className="relative z-20 w-full max-w-7xl mx-auto pt-32 pb-16 px-6 lg:px-12">
        ...
        {/* Header - Wing Mentor Procedures Manual */}
        ...
        {/* NEW SECTION: THE VAULT RESOURCES */}
        ...
        {/* Content Sections - Digitized from Manual */}
